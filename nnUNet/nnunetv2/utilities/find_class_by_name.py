...
def recursive_find_python_class(
    ...
    If `class_name` is not found under `folder`, falls back to scanning the directories listed
    in the `nnUNet_extTrainer` environment variable (os.pathsep-separated), as documented in
    documentation/how-to/share-models-with-custom-trainers.md. Each ext directory is added to
    sys.path and its imports cleaned up on exit so successive lookups stay isolated.
    ...
    cleanup_imports_from_base_folder : bool, optional
        If True, modules imported from `base_folder` are removed from sys.modules on exit.
    ...
    # nnUNet_extTrainer fallback — search user-supplied directories listed in the
    # env var (os.pathsep-separated). Each dir is added to sys.path for the scope
    # of the scan and modules it loaded are cleaned up on exit.
    for ext in filter(None, os.environ.get('nnUNet_extTrainer', '').split(os.pathsep)):
        if not os.path.isdir(ext):
            continue
        with ExitStack() as stack:
            stack.enter_context(temporarily_extend_syspath(ext))
            stack.enter_context(temporarily_cleanup_imports_from_path(ext))
            if verbose:
                print(f"nnUNet_extTrainer fallback: scanning {ext}")
            result = _recursive_find_python_class(
                ext,
                class_name,
                current_module=None,
