...
from fastsegmentator._vendor.nnunetv2.paths import nnUNet_raw
...
def log_runtime(func):
    def wrapper(*args, **kwargs):
        ...
        if class_name:
            logging.getLogger().info(f"Function {class_name}.{function_name} executed in {runtime:.6f} seconds")
        else:
            logging.getLogger().info(f"Function {function_name} executed in {runtime:.6f} seconds")
        return result
    return wrapper
